...
const PieceDescription = (Props: PieceDescriptionProps) => {
  const { t } = useTranslation('translation', { i18n });
  const { addPiece, type } = Props;
    ...
    case PieceType.commoner:
      return (
        <div className='padded'>
          ...
          <p><b>{t('Speed')}</b> 1 <i>(cell per turn, forward movements only)</i></p>
          <b>{t('Abilities')}</b>
    ...
    case PieceType.smuggler:
      return (
        <div className='padded'>
          ...
          <p><b>{t('Speed')}</b> 4 <i>(cells per turn)</i></p>
          <b>{t('Abilities')}</b>
          <p>When you move a smuggler, you can choose to transport up to 4 other pieces
            . The other pieces must be on the same cell as the smuggler.
          </p>
